...
class _DesignerPage2State extends State<DesignerPage2> {
  ...
  // 🚀 MAGIC: Naye 'master_inventory' database se data fetch karna
  Future<void> _fetchPlys() async {
    try {
      final snap = await FirebaseFirestore.instance
          .collection("master_inventory") // ✅ CHANGED: Ab master_inventory se padhega
          .get();

      final items = snap.docs
          .map((doc) => (doc.data()['Plys'] ?? '').toString()) // 'Plys' category ka data uthayega
          .where((val) => val.isNotEmpty)
          .toList();

      setState(() {
        // toSet() duplicates hatayega, aur sort() A-Z lagayega
        _plyItems = ["No", ...items.toSet().toList()..sort()];
        _loadingPlys = false;
      });
      debugPrint("✅ Fetched Plys from Master Inventory");
    } catch (e) {
          ...
          children: [
            ...
            if (form.canView("PlyType")) ...[
              _loadingPlys
                ...
                // 🚀 CHANGED: Agar koi naya item add kare, toh master_inventory mein bhej do
                firestoreCollection: "master_inventory",
                firestoreField: "Plys",
