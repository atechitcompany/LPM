...
class _DesignerPage3State extends State<DesignerPage3> {
  ...
  // 🚀 UPDATE 1: Fetch Blades from master_inventory
  Future<void> _fetchBlades() async {
    try {
      ...
      final items = snap.docs
          .map((doc) => (doc.data()['Blades'] ?? '').toString()) // ✅ MUST match the field name in Firestore
          .where((val) => val.isNotEmpty)
          .toList();

      setState(() {
        _bladeItems = ["No", ...items.toSet().toList()..sort()];
  ...
  // 🚀 UPDATE 2: Fetch Creasings from master_inventory
  Future<void> _fetchCreasings() async {
    try {
      ...
      final items = snap.docs
          .map((doc) => (doc.data()['Creasings'] ?? '').toString()) // ✅ MUST match the field name in Firestore
          .where((val) => val.isNotEmpty)
          .toList();

      setState(() {
        _creasingItems = ["No", ...items.toSet().toList()..sort()];
  ...
  // 🚀 UPDATE 3: Fetch Capsules from master_inventory
  Future<void> _fetchCapsules() async {
    try {
      ...
      final items = snap.docs
          .map((doc) => (doc.data()['Capsules'] ?? '').toString()) // ✅ MUST match the field name in Firestore
          .where((val) => val.isNotEmpty)
          .toList();

      setState(() {
        _capsuleItems = ["No", ...items.toSet().toList()..sort()];
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            /// ✅ Blade (Dropdown is now SearchableDropdownWithInitial, doesn't support adding directly in this specific widget implementation you provided)
            ...
            if (form.canView("CapsuleType")) ...[
              _loadingCapsules
                ...
                firestoreField: "Capsules",               // ✅ UPDATED
                onChanged: (v) {
