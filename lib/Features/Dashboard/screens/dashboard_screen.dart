...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void showAddDialog() {
    showDialog(
      ...
      builder: (_) => AddActivityDialog(
        ...
        onStatusChanged: (val) => setState(() => selectedStatus = val ?? 'Hot'),
        onAdd: addActivity,
      ),
    );
  }
  ...
  void _openSortMenu(TapDownDetails details) {
    ...
    entry = WindowsPopupMenu.show(
      context: context,
      position: Rect.fromLTWH(tapPos.dx, tapPos.dy, 0, 0),
      children: [
        _menuHeader("SORT BY"),
        ...
        _submenuItem("Group By", Icons.group, () {
          _openGroupBySubMenu(tapPos);
          removeMenu();
        }),
      ],
    );
  }
  void _openGroupBySubMenu(Offset position) {
    late OverlayEntry entry;
